...
class Server:
  ...
  def _maketemplate(self, path: str) -> str:
    return "^" + re.sub(r"/{([a-zA-Z0-9_]+)}", "/(?P<\\g<1>>[a-zA-Z0-9_]+)", path.replace("*", ".*"), 0) + "$"

  async def _run(self):
    await self.server.serve()

  def _makehandler(self):
    async def _fakehandler(scope: HTTPScope, receive: ASGIReceiveCallable, send: ASGISendCallable):
      await self._handler(scope, receive, send)
    return _fakehandler

  async def _handler(self, scope: HTTPScope, receive: ASGIReceiveCallable, send: ASGISendCallable):
    if scope["type"] != "http":
      return
  ...
  def use(self, path: str):
    def subdecorator(func: Callable[[Request], Awaitable[Response | type[Skip]]]):
      self.middlewares.append(Middleware(self, "*", self._maketemplate(path), func))
      return func
    return subdecorator

  def get(self, path: str):
    def subdecorator(func: Callable[[Request], Awaitable[Response | type[Skip]]]):
      self.middlewares.append(Middleware(self, "GET", self._maketemplate(path), func))
      return func
    return subdecorator

  def post(self, path: str):
    def subdecorator(func: Callable[[Request], Awaitable[Response | type[Skip]]]):
      self.middlewares.append(Middleware(self, "POST", self._maketemplate(path), func))
      return func
    return subdecorator

  def put(self, path: str):
    def subdecorator(func: Callable[[Request], Awaitable[Response | type[Skip]]]):
      self.middlewares.append(Middleware(self, "PUT", self._maketemplate(path), func))
      return func
    return subdecorator

  def patch(self, path: str):
    def subdecorator(func: Callable[[Request], Awaitable[Response | type[Skip]]]):
      self.middlewares.append(Middleware(self, "PATCH", self._maketemplate(path), func))
      return func
    return subdecorator

  def delete(self, path: str):
    def subdecorator(func: Callable[[Request], Awaitable[Response | type[Skip]]]):
      self.middlewares.append(Middleware(self, "DELETE", self._maketemplate(path), func))
      return func
    return subdecorator
  ...
  def static(self, prefix: str, path: str):
    async def _middleware(request: Request):
      ...
      if request.path == prefix or request.path.startswith(prefix + "/"):
        currentPath = os.path.join(rootPath, request.path.removeprefix(prefix).replace("/..", "").replace("/.", "").lstrip("/"))
...
class Request:
  def __init__(self, server: Server, scope: HTTPScope, receive: ASGIReceiveCallable, params: dict[str, str]):
    self.server = server
    self._scope = scope
    ...
    for query in queries:
      ...
      self.query[name] = True if amount < 2 else urllib.parse.unquote("=".join(parts))
  ...
  async def json(self) -> dict[str, str | bool]:
    if self.headers["content-type"] == "application/x-www-form-urlencoded":
      ...
      for query in queries:
        ...
        amount = len(parts)
        name = urllib.parse.unquote(parts.pop(0))
        body[name] = True if amount < 2 else urllib.parse.unquote("=".join(parts))
      return body
    else:
      return json.loads(await self.text())

class Response:
  def __init__(self, request: Request, status: int = 200, headers: dict[str, str | int] = {}):
...
class Middleware:
  def __init__(self, server: Server, method: str, path: str, handler: Callable[[Request], Awaitable[Response | type[Skip]]]):
    self.server = server
    self.method = method
    self.path = path
